...
do {                                                                                           \
    ...
    public:
        ...
        template<typename T,
                 typename... Args,
                 typename = std::enable_if_t<std::conjunction_v<std::is_same_v<T, Args>...>>>
        void set_uniform(GLuint const uniform_id, Args&& ...args) {
            constexpr std::size_t args_cnt{sizeof...(args)};
            static_assert(args_cnt < 4, "Unable to set more then 4 parameters to uniform");


#define UNIFORM_CASE_BLOCK(cnt, last_letter)                       \
            case cnt:                                                \
                glUniform ##args_cnt ##last_letter (uniform_id, args...); \
                break

#define UNIFORM_GENERIC_SWITCH_BLOCK(args_cnt, last_letter) \
            switch(args_cnt) { \
                    UNIFORM_CASE_BLOCK(1, last_letter); \
                    UNIFORM_CASE_BLOCK(2, last_letter); \
                    UNIFORM_CASE_BLOCK(3, last_letter); \
                    UNIFORM_CASE_BLOCK(4, last_letter); \
                    default:                            \
                        static_assert("Wrong number of arguments"); \
            }

            if constexpr (std::is_same_v<T, GLfloat>) {
                UNIFORM_GENERIC_SWITCH_BLOCK(args_cnt, f);
            } else if constexpr (std::is_same_v<T, GLint>) {
                UNIFORM_GENERIC_SWITCH_BLOCK(args_cnt, i);
            } else
                static_assert("Unsupported type");

#undef UNIFORM_GENERIC_SWITCH_BLOCK
#undef UNIFORM_CASE_BLOCK
    ...
    public:
        ...
        template<typename T>
        static void set_context(T&& window) {
            context_window = std::forward<T>(window);
